...
/////////////////////////////////////////////////////////////////////////////
// car body scaled then translated
void drawCarbody(float width, float height, float length) {
  float h = height / 2;
  float b = 4.0; // bottom elevation of car
  // bottom
  glPushMatrix();
  glTranslatef(0.0, 0.0, b);        // move up bottom by a bit
  glScalef(length, width - 2.0, h); // facing x direction, z is top
  glutSolidCube(1.0f);
  glPopMatrix();
  // top
  glPushMatrix();
  glTranslatef(0.0, 0.0, h + b);
  glScalef(length / 2, width - 2.0, h);
  glutSolidCube(1.0f);
  glPopMatrix();
}

// tires rotated then translated
void drawWheels(float width, float height, float length) {
  float innerRadius = height / 8;
  float outerRaidus = height / 4;
  glPushMatrix();
  glTranslatef(length / 4, width / 2, height / 4);
  glRotatef(90.0, 1.0, 0.0, 0.0);
  glutSolidTorus(innerRadius, outerRaidus, 16, 16);
  glPopMatrix();
...
void DrawOneCar(float bodyColor[3]) {
  ...
  // Draw the car body.
  //****************************
  // drawSolidBox(CAR_WIDTH, CAR_HEIGHT, CAR_LENGTH);
  drawCarbody(CAR_WIDTH, CAR_HEIGHT, CAR_LENGTH);
  ...
  // Draw the four tyres.
  //****************************
  drawWheels(CAR_WIDTH, CAR_HEIGHT, CAR_LENGTH);
  glFlush();
}
...
void MyDisplay(void) {
  ...
  double y = eyeDistance * sin(eyeLatitude * PI / 180);
  double z =
