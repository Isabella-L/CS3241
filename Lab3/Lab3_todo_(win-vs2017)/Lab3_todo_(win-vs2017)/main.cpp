...
const char customTopFile[] = "images/cube_top.jpg";
const char customBottomFile[] = "images/cube_bottom.jpg";
const char customSidesFile[] = "images/cube_side.jpg";
...
// custom object
GLuint customTopObj;
GLuint customBottomObj;
GLuint customSidesObj;
...
////////////////////////https://youtube.com/watch?v=0t4BEpbw5jQhttps://youtube.com/watch?v=0t4BEpbw5jQ/////////////////////////////////////////////////////
...
void MakeReflectionImage(void) {
  ...
  //****************************
  // STEP 1: Clear the color and depth buffers.
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  // STEP 2: Set up the correct view volume for the imaginary viewpoint.
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  glFrustum(-eyePos[1] + TABLETOP_Y1, -eyePos[1] + TABLETOP_Y2,
            -eyePos[0] + TABLETOP_X1, -eyePos[0] + TABLETOP_X2,
            eyePos[2] - TABLETOP_Z, eyePos[2] - TABLETOP_Z + SCENE_RADIUS);

  // STEP 3: Set up the imaginary viewpoint.
  glMatrixMode(GL_MODELVIEW);
  glLoadIdentity();
  gluLookAt(eyePos[0], eyePos[1], eyePos[2] - (2 * (eyePos[2] - TABLETOP_Z)),
            eyePos[0], eyePos[1], eyePos[2], 1.0, 0.0, 0.0);
  ...
  // STEP 5: Draw the scene. You don't need to draw all objects, just the ones
  // you want to reflect. Draw the table with reflection texture.
  // DrawCustomObject();
...
void SetUpTextureMaps(void) {
  ...
  // custom obj top
  glGenTextures(1, &customTopObj);
  glBindTexture(GL_TEXTURE_2D, customTopObj);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
  ...
  if (ReadImageFile(customTopFile, &imageData, &imageWidth, &imageHeight,
                    &numComponents) == 0)
    exit(1);
  if (numComponents != 3) {
    fprintf(stderr, "Error: Texture image is not in RGB format.\n");
  ...
  // custom obj bottom
  glGenTextures(1, &customBottomObj);
  glBindTexture(GL_TEXTURE_2D, customBottomObj);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
  ...
  if (ReadImageFile(customBottomFile, &imageData, &imageWidth, &imageHeight,
                    &numComponents) == 0)
    exit(1);
  if (numComponents != 3) {
    fprintf(stderr, "Error: Texture image is not in RGB format.\n");
  ...
  // custom obj sides
  glGenTextures(1, &customSidesObj);
  glBindTexture(GL_TEXTURE_2D, customSidesObj);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
  ...
  if (ReadImageFile(customSidesFile, &imageData, &imageWidth, &imageHeight,
                    &numComponents) == 0)
    exit(1);
  if (numComponents != 3) {
    fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
void DrawTable(void) {
  ...
  glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
  glBindTexture(GL_TEXTURE_2D, reflectionTexObj);
  glNormal3f(0.0, 0.0, 0.1); // Normal vector.
  SubdivideAndDrawQuad(24, 24, 0.0, 0.0, TABLETOP_X1, TABLETOP_Y1, TABLETOP_Z,
                       0.0, 1.0, TABLETOP_X2, TABLETOP_Y1, TABLETOP_Z, 1.0, 1.0,
                       TABLETOP_X2, TABLETOP_Y2, TABLETOP_Z, 1.0, 0.0,
...
void DrawCustomObj(void) {
  ...
  glNormal3f(0.0, 0.0, 1.0); // Normal vector.
  SubdivideAndDrawQuad(24, 24, 0.0, 0.0, -x, -y, TABLETOP_Z + height, 0.0, 1.0,
                       x, -y, TABLETOP_Z + height, 1.0, 1.0, x, y,
                       TABLETOP_Z + height, 1.0, 0.0, -x, y,
                       TABLETOP_Z + height);

  glBindTexture(GL_TEXTURE_2D, customBottomObj);
  glNormal3f(0.0, 0.0, -1.0); // Normal vector.
  SubdivideAndDrawQuad(24, 24, 0.0, 0.0, -x, -y, TABLETOP_Z + z, 1.0, 0.0, -x,
                       y, TABLETOP_Z + z, 1.0, 1.0, x, y, TABLETOP_Z + z, 0.0,
                       1.0, x, -y, TABLETOP_Z + z);

  // sides
  glBindTexture(GL_TEXTURE_2D, customSidesObj);
  // In +y direction.
  glNormal3f(0.0, 1.0, 0.0); // Normal vector.
  SubdivideAndDrawQuad(24, 2, 0.0, 0.0, x, y, TABLETOP_Z + z, 1.0, 0.0, -x, y,
                       TABLETOP_Z + z, 1.0, 1.0, -x, y, TABLETOP_Z + height,
                       0.0, 1.0, x, y, TABLETOP_Z + height);
  // In -y direction.
  glNormal3f(0.0, -1.0, 0.0); // Normal vector.
  SubdivideAndDrawQuad(24, 2, 0.0, 0.0, -x, -y, TABLETOP_Z + z, 1.0, 0.0, x, -y,
                       TABLETOP_Z + z, 1.0, 1.0, x, -y, TABLETOP_Z + height,
  ...
  glNormal3f(1.0, 0.0, 0.0); // Normal vector.
  SubdivideAndDrawQuad(24, 2, 0.0, 0.0, x, -y, TABLETOP_Z + z, 1.0, 0.0, x, y,
                       TABLETOP_Z + z, 1.0, 1.0, x, y, TABLETOP_Z + height, 0.0,
                       1.0, x, -y, TABLETOP_Z + height);
  // In -x direction.
  glNormal3f(-1.0, 0.0, 0.0); // Normal vector.
  SubdivideAndDrawQuad(24, 2, 0.0, 0.0, -x, y, TABLETOP_Z + z, 1.0, 0.0, -x, -y,
                       TABLETOP_Z + z, 1.0, 1.0, -x, -y, TABLETOP_Z + height,
